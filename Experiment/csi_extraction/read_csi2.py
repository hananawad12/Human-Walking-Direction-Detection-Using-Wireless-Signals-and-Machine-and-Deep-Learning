...
def unpack_csi_struct(f, endianess='>'):  # Big-Endian as Default Value
    csi_inf = csi_struct()
    csi_inf.bw = struct.unpack(endianess + 'H', f.read(2))[0]  # Block Length     2Byte
    csi_inf.timestamp = struct.unpack(endianess + 'Q', f.read(8))[0]  # TimeStamp      8Byte
    csi_inf.channel = struct.unpack(endianess + 'H', f.read(2))[0]  # csi_len        2Byte
    csi_inf.csi_len = struct.unpack(endianess + 'H', f.read(2))[0]  # tx             2Byte
    ...
    csi_inf.num_tones = struct.unpack(endianess + 'B', f.read(1))[0]  # bandWidth      1Byte
    csi_inf.nr = struct.unpack(endianess + 'B', f.read(1))[0]  # num_tones      1Byte
    csi_inf.nc = struct.unpack(endianess + 'B', f.read(1))[0]  # nr             1Byte
    csi_inf.rssi = struct.unpack(endianess + 'B', f.read(1))[0]  # nc             1Byte
    csi_inf.rssi1 = struct.unpack(endianess + 'B', f.read(1))[0]  # rate           1Byte
    csi_inf.rssi2 = struct.unpack(endianess + 'B', f.read(1))[0]  # rssi           1Byte
    csi_inf.rssi3 = struct.unpack(endianess + 'B', f.read(1))[0]  # rssi1          1Byte
    csi_inf.rate = struct.unpack(endianess + 'B', f.read(1))[0]  # rssi2          1Byte
    csi_inf.field_len= struct.unpack(endianess + 'B', f.read(1))[0]  # rssi3          1Byte
